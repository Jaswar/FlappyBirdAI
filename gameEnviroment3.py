...
class Enviroment:
    def __init__(self, waitTime):
        ...
        self.upperPipes = [
            ...
            {'x': SCREENWIDTH + 180, 'y': newPipe2[0]['y']},
        ]
        self.lowerPipes = [
            {'x': SCREENWIDTH, 'y': newPipe1[1]['y']},
            {'x': SCREENWIDTH + 180, 'y': newPipe2[1]['y']},
        ]
        ...
        self.next_pipe_hole_y = (self.lowerPipes[0]['y'] + (
                    self.upperPipes[0]['y'] + IMAGES['pipe'][0].get_height())) / 2  # wartość y dziury w rurze
    ...
    def step(self,action):
        ...
        for i in range(PREV_FRAMES):
            observation, reward, isGameOver, _ = self.insidestep(action)
            rew += reward
            if isGameOver == True:
                gOver = True
                
            for j in range(len(observation)):
                obs.append(observation[j])
        ...
        return obs, rew, gOver, _



    def insidestep(self, action):
        pygame.event.pump()
        # self.playery, self.next_pipe_x, self.next_pipe_hole_y = self.state
        self.next_pipe_x = self.upperPipes[0]['x'] - PIPE_WIDTH
        self.next_pipe_bottom = self.lowerPipes[0]['y']
        self.next_pipe_upper = self.upperPipes[0]['y'] + IMAGES['pipe'][0].get_height()

        
        reward = 0.05  # Nagroda za przeżycie
        ...
        for pipe in self.upperPipes:
            ...
            if pipeMidPos <= playerMidPos < pipeMidPos + 4:      #Zmiany
                self.score += 1
                ###Zmiana skupienia na rury
                
                reward = 1  # Jeżeli tak to nagroda duża
      ...
      #  if 0 < self.upperPipes[0]['x'] < 5 and self.upperPipes[0]['x'] < -PIPE_WIDTH:
           # newPipe = getRandomPipe()
          #  self.upperPipes.append(newPipe[0])
          #  self.lowerPipes.append(newPipe[1])
        ...
        if len(self.upperPipes) >1:
            if self.next_pipe_x < 0:
                self.next_pipe_x = self.upperPipes[1]['x'] - PIPE_WIDTH + 2
                self.next_pipe_bottom = self.lowerPipes[1]['y'] 
                self.next_pipe_upper = self.upperPipes[1]['y'] + IMAGES['pipe'][0].get_height() 
        
        if len(self.upperPipes) == 1:
            self.next_pipe_x = self.upperPipes[0]['x'] - PIPE_WIDTH + 2 
            self.next_pipe_bottom = self.lowerPipes[0]['y'] 
            self.next_pipe_upper = self.upperPipes[0]['y'] + IMAGES['pipe'][0].get_height()
        
        #print(self.next_pipe_x, self.next_pipe_bottom, self.next_pipe_upper)
        ...
        nextPipe = nextPipe /SCREENWIDTH  # Max pipe distance from player will be 450
        playerV = self.playerVelY
        playerV = playerV / self.playerMaxVelY
        nextBottom = self.next_pipe_bottom
        nextBottom = min(SCREENHEIGHT, nextBottom) / SCREENHEIGHT
        nextUpper = self.next_pipe_upper
        nextUpper = min(SCREENHEIGHT, nextUpper) / SCREENHEIGHT

        self.state = (height, nextPipe, playerV, nextBottom, nextUpper)
...
def getRandomPipe():
    ...
    gapYs = [10 ,30 ,50, 70 , 90]      #Zmniejszyć ilosc opcji do np.5
